...
g = sns.FacetGrid(df, row="alpha", col="beta", hue="agent_label", margin_titles=True, height=7.5, palette=palette_dict)
...
plot_overlay_pA_grid <- function(cfg_list, label_list, root_list = c("data", "expected_value")) {
  ...
  for (i in seq_along(cfg_list)) {
    ...
    folder   <- sprintf("T%s_r%s_%sagents_rep%s",
                        fstr(cfg$T_val),
                        paste(cfg$r_base, collapse = "-"),
                        cfg$n_agents,
                        cfg$rep_idx)
    csv_dir  <- file.path(root, folder, "csv")
    agent_cli <- cfg$agent_type
    
    df_list <- list()
    
    for (alpha in cfg$alphas) {
      for (beta in cfg$betas) {
        t_label <- paste0("p", fstr(cfg$extra_p))
        fname   <- sprintf("%s_%s_a%s_b%s_pA.csv",
                           agent_cli, t_label,
                           fstr(alpha), fstr(beta))
        fpath   <- file.path(csv_dir, fname)
        ...
        if (file.exists(fpath)) {
          ...
          warning("ファイルが存在しません: ", fpath)
        }
      }
    }
    
    df_all_total[[i]] <- dplyr::bind_rows(df_list)
  }
  
  df_pA_all <- dplyr::bind_rows(df_all_total) %>%
    dplyr::mutate(
      alpha = factor(alpha, levels = sort(unique(cfg_list[[1]]$alphas))),
      beta  = factor(beta,  levels = sort(unique(cfg_list[[1]]$betas))),
      sample_size = factor(sample_size, levels = label_list)
    )
  
    ggplot2::ggplot(df_pA_all, ggplot2::aes(x = trial, y = pA,
                                          color = sample_size,
                                          group = interaction(sample_size, alpha, beta))) +
    ggplot2::geom_line(linewidth = 0.8) +
    ggplot2::facet_grid(alpha ~ beta,
...
### 3.3.1 50人のデータ
```{r hyouzi}
plot_overlay_pA_grid(
  cfg_list   = list(cfg, cfg_ev),
  label_list = c("n=50", "n=5000"),
  root_list  = c("data", "expected_value")
)
```
...
fit_model <- function(cfg, alpha, beta, stan_file = "first_value.stan") {
...
alpha_beta_summary <- imap_dfr(
  fit_list,
  ~ .x$summary(variables = c("alpha", "beta","Q_init1")) %>%
      as_tibble() %>%
      mutate(pattern = .y) %>%                      
      select(
        pattern,
...
param_list <- c("alpha", "beta","Q_init1")
...
#### 5.1.1.4 traceplot Q_init1
```{r Qinit1,cache=FALSE}
plot_trace_facet("Q_init1", fit_list)
```
...
plot_density_facet("alpha", fit_list, y_max = 50)
```
...
#### 5.2.1.3 Q_init1
```{r zigoQ_init1, cache=FALSE}
plot_density_facet("Q_init1", fit_list, y_max = 10)
``` 
...
for (alpha in cfg$alphas) {
  for (beta in cfg$betas) {
    t_label   <- paste0("p", fstr(cfg$extra_p))
    file_name <- sprintf("%s_%s_a%s_b%s_pA.csv",
      cfg$agent_type,  
      t_label,
      fstr(alpha),
      fstr(beta)
...
df_pA_all <- dplyr::bind_rows(df_all) %>%
  dplyr::mutate(
    alpha = factor(alpha, levels = sort(unique(cfg$alphas))),
    beta  = factor(beta,  levels = sort(unique(cfg$betas)))
  )

ggplot2::ggplot(df_pA_all, ggplot2::aes(x = trial, y = pA, group = interaction(alpha, beta))) +
  ggplot2::geom_line(color = "orange", linewidth = 0.7) +
  ggplot2::facet_grid(alpha ~ beta, labeller = ggplot2::label_both, switch = "y") +
  ggplot2::coord_cartesian(ylim = c(0, 1)) +
  ggplot2::theme_minimal(base_size = 13) +
  ggplot2::theme(
    strip.placement   = "outside",
...
df_ppc_all <- imap_dfr(fit_list, function(fit, pat) {
  ...
  pA_summary <- t(sapply(seq_len(Tt), function(t) {
    sel <- which(idx[,3] == t)
    x   <- as.vector(arr[,, sel, drop = FALSE])
    c(
      mean  = mean(x),
  ...
  colnames(pA_summary) <- c("mean","lower","upper")
  ...
  tibble(
    trial = seq_len(Tt) - 1,
    pA    = pA_summary[,"mean"],
    lower = pA_summary[,"lower"],
    upper = pA_summary[,"upper"],
